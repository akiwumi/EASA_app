...
export default function Composer() {
  ...
  return (
    <form onSubmit={submit} className="flex shrink-0 gap-2 border-t border-[var(--easa-color-border)] bg-white p-3">
      <label className="sr-only" htmlFor="coworker-message">Message Henry</label>
      <textarea id="coworker-message" value={content} onChange={(event) => setContent(event.target.value)} placeholder="Ask Henry about manuals or findings..." rows={2} className="easa-input min-h-[52px] flex-1 resize-none" />
