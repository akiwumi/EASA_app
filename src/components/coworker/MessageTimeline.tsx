  ...
  return (
    <div className="flex-1 space-y-3 overflow-y-auto p-4">
      {messages.map((message) => (
        <div key={message.id} className={message.role === "user" ? "ml-8 rounded-xl bg-[var(--easa-color-brand-primary)] p-3 text-sm text-white" : "mr-4 rounded-xl border border-[var(--easa-color-border)] bg-[var(--easa-color-surface-2)] p-3 text-sm text-[var(--easa-color-text-primary)]"}>
