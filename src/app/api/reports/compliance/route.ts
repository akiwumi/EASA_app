...
function wrapLine(line: string, max = 94): PdfLine[] {
...
function buildPages(lines: PdfLine[]) {
  const pages: PdfLine[][] = [];
  let current: PdfLine[] = [];
...
export async function GET(request: Request) {
  ...
  const lines: PdfLine[] = [];
  lines.push({ text: "Compliance Audit Report", bold: true });
  lines.push({ text: "" });
  lines.push({ text: `School: ${schoolName}` });
  lines.push({ text: `Reporting period: ${startIso.slice(0, 10)} to ${endIso.slice(0, 10)}` });
  lines.push({ text: `Generated date: ${now.toISOString().slice(0, 10)}` });
  lines.push({ text: `Generated by: ${generatedBy}` });
  lines.push({ text: "" });
  lines.push({ text: "Summary", bold: true });
  lines.push({ text: `- Total regulation changes monitored: ${Number(totalRegChanges ?? 0)}` });
  lines.push({ text: `- Total findings generated: ${Number(totalFindings ?? 0)}` });
  lines.push({ text: `- Total approved: ${approvedCount}` });
  lines.push({ text: `- Total dismissed with reason: ${dismissedCount}` });
  lines.push({ text: `- Total pending: ${pendingCount}` });
  lines.push({ text: "" });
  lines.push({ text: "Approved changes", bold: true });
  if (approvedCount === 0) {
    lines.push({ text: "No approved changes in this reporting period." });
  } else {
    for (const approval of approvals ?? []) {
      ...
      lines.push({ text: `  Approved by: ${userNameMap.get(String(approval.approver_id)) ?? "Unknown"} on ${String(approval.created_at).slice(0, 19).replace("T", " ")} UTC` });
    }
  }
  lines.push({ text: "" });
  lines.push({ text: "Dismissed with reason", bold: true });
  if (dismissedCount === 0) {
    lines.push({ text: "No dismissed findings in this reporting period." });
  } else {
    for (const finding of dismissedFindings ?? []) {
      ...
      lines.push({ text: `  Dismissed by: ${userNameMap.get(String(finding.dismissed_by)) ?? "Unknown"} on ${String(finding.dismissed_at).slice(0, 19).replace("T", " ")} UTC` });
      lines.push(...wrapLine(`  Reason: ${(finding.dismissal_reason as string | null) ?? "No reason provided"}`));
    }
  }
  lines.push({ text: "" });
  lines.push({ text: "Pending", bold: true });
  if (pendingCount === 0) {
    lines.push({ text: "No pending items as of report date." });
  } else {
    for (const pending of pendingUpdates ?? []) {
      ...
      lines.push({ text: `* ${heading} · ${(pending.risk_level as string | null) ?? "unknown"} risk` });
